...
func _build_interface() -> void:
    ...
    var brand_row:=HBoxContainer.new(); brand_row.add_theme_constant_override("separation",10); content.add_child(brand_row)
    var brand_icon:=TextureRect.new(); brand_icon.texture=preload("res://assets/branding/broken_knight_icon_256_v1.png"); brand_icon.custom_minimum_size=Vector2(42,42); brand_icon.expand_mode=TextureRect.EXPAND_IGNORE_SIZE; brand_icon.stretch_mode=TextureRect.STRETCH_KEEP_ASPECT_CENTERED; brand_icon.mouse_filter=Control.MOUSE_FILTER_IGNORE; brand_row.add_child(brand_icon)
    var title:=Label.new(); title.text="BROKEN KNIGHT  •  ADMIN TOOLS"; title.add_theme_font_size_override("font_size",24); title.add_theme_color_override("font_color",Color(1,.81,.35)); title.size_flags_vertical=Control.SIZE_SHRINK_CENTER; brand_row.add_child(title)
